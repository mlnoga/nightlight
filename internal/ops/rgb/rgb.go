...
func (op *OpRGBCombine) MakePromises(ins []ops.Promise, c *ops.Context) (outs []ops.Promise, err error) {
	if len(ins) < 3 || len(ins) > 4 {
		return nil, fmt.Errorf("%s operator with %d inputs", op.Type, len(ins))
	}
...
func (op *OpRGBCombine) Apply(fs []*fits.Image, c *ops.Context) (fOut *fits.Image, err error) {
	if len(fs) < 3 || len(fs) > 4 {
		return nil, fmt.Errorf("invalid number of channels for color combination: %d", len(fs))
	}
	if len(fs) == 4 {
		c.LumFrame = fs[3]
	}
...
func NewOpRGBBalanceDefault() *OpRGBBalance {
	return NewOpRGBBalance(16, 0.1, 0, 0.75, fits.RGB{R: 1, G: 1, B: 1}, fits.RGB{R: 1, G: 1, B: 1})
}
